...
aov.S1
#summary(aov.S1) 
etasq(aov.S1, anova = TRUE) # Provides Eta^2 value
```

Calculate separate ANOVA for each direction- old code
```{r}
#S1_data_east <- filter(S1_Subway, DIRECTION == "EAST" )
...
```{r}
S1_data_STG <- filter(S1_Subway, STN_NAME == "STG" )
aov.S1_STG <- aov (DISTANCE~DIRECTION, data=S1_data_STG)
aov.S1_STG
summary(aov.S1_e)
etasq(aov.S1_STG)
```
...
```{r}
S1_data_SPAD <- filter(S1_Subway, STN_NAME == "SPAD" )
aov.S1_SPAD <- aov (DISTANCE~DIRECTION, data=S1_data_SPAD)
aov.S1_SPAD
summary(aov.S1_SPAD)
etasq(aov.S1_SPAD)
```
...
```{r}
TukeyHSD(aov.S1) #calculates p statistics found for Direction and Station Name
```


# Step 5: Conclusion

The study replicated some findings and not others. The most glaring failure to replicate was in Bloor-Yonge Station classifed by direction, wherein the authors claimed significance and my analysis did yielded a p value of aroud 0.5.
